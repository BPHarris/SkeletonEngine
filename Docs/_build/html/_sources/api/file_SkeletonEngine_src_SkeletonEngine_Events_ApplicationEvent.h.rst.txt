
.. _file_SkeletonEngine_src_SkeletonEngine_Events_ApplicationEvent.h:

File ApplicationEvent.h
=======================

|exhale_lsh| :ref:`Parent directory <dir_SkeletonEngine_src_SkeletonEngine_Events>` (``SkeletonEngine\src\SkeletonEngine\Events``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS


:ref:`namespace_SkeletonEngine` Application events. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``SkeletonEngine\src\SkeletonEngine\Events\ApplicationEvent.h``)
----------------------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_SkeletonEngine_src_SkeletonEngine_Events_ApplicationEvent.h.rst



Detailed Description
--------------------

BP Harris 

July 2020 






Includes
--------


- ``Event.h`` (:ref:`file_SkeletonEngine_src_SkeletonEngine_Events_ApplicationEvent.h`)



Included By
-----------


- :ref:`file_SkeletonEngine_src_Platform_Windows_WindowsWindow.cpp`

- :ref:`file_SkeletonEngine_src_SkeletonEngine_Application.h`




Namespaces
----------


- :ref:`namespace_SkeletonEngine`


Classes
-------


- :ref:`exhale_class_class_skeleton_engine_1_1_app_render_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_app_tick_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_app_update_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_window_closed_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_window_focused_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_window_lost_focus_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_window_moved_event`

- :ref:`exhale_class_class_skeleton_engine_1_1_window_resized_event`

