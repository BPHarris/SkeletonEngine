
.. _file_SkeletonEngine_src_SkeletonEngine_Application.h:

File Application.h
==================

|exhale_lsh| :ref:`Parent directory <dir_SkeletonEngine_src_SkeletonEngine>` (``SkeletonEngine\src\SkeletonEngine``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS


Basic SkeletonEngine Application interface. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``SkeletonEngine\src\SkeletonEngine\Application.h``)
----------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_SkeletonEngine_src_SkeletonEngine_Application.h.rst



Detailed Description
--------------------

Brandon Harris (bpharris@pm.me) 

July 2020 






Includes
--------


- ``Core.h`` (:ref:`file_SkeletonEngine_src_SkeletonEngine_Core.h`)

- ``Events/ApplicationEvent.h``

- ``Events/Event.h``

- ``LayerStack.h`` (:ref:`file_SkeletonEngine_src_SkeletonEngine_LayerStack.h`)

- ``Window.h`` (:ref:`file_SkeletonEngine_src_SkeletonEngine_Window.h`)



Included By
-----------


- :ref:`file_SkeletonEngine_src_SkeletonEngine_Application.cpp`

- :ref:`file_SkeletonEngine_src_SkeletonEngine.h`




Namespaces
----------


- :ref:`namespace_SkeletonEngine`


Classes
-------


- :ref:`exhale_class_class_skeleton_engine_1_1_application`


Functions
---------


- :ref:`exhale_function__application_8h_1a3ecba78e8136f5fde0386b52489c170d`

